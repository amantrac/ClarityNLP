...
In production, for each class we will store the corresponding threshold in memory an assign the class only if the predicted score fall above the threshold. By so doing, we will keep a precision of 99% or higher at the cost of coverge, ffor instance for category 2 we will nearly never assign it. 
That's where one would need to dive in the false positives and false negatives examples of the category 2 in order to understand why the system if not behaving that well (by for instance labeling additional example, or if in case of logistic model looking at the features themselves).</font>
